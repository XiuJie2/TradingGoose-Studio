...
import { resolveMinimaxServiceConfig } from '@/lib/system-services/runtime'
...
export const minimaxProvider: ProviderConfig = {
  id: 'minimax',
  name: 'MiniMax',
  description: "MiniMax's M-series models via their OpenAI-compatible API",
  version: '1.0.0',
  ...
  executeRequest: async (
    ...
    const payload: any = {
      ...
      // Thinking cannot be turned off on the M2.x models, and by default it comes
      // back inside `content` wrapped in `<think>` tags. This moves it to a
      // separate `reasoning_content` field so callers get a clean answer.
      ...
      while (iterationCount < MAX_ITERATIONS) {
        ...
        for (const toolCall of toolCallsInResponse) {
          try {
            ...
            logger.error('Error processing tool call (MiniMax):', {
