...
export const nvidiaProvider: ProviderConfig = {
  ...
  executeRequest: async (
    ...
    // Model ids are namespaced in the picker but sent to NIM unprefixed, e.g.
    // `nvidia/meta/llama-3.3-70b-instruct` -> `meta/llama-3.3-70b-instruct`.
    const apiModel = (request.model || '').replace(/^nvidia\//, '')

    logger.info('Preparing NVIDIA request', {
      model: apiModel,
    ...
    if (tools?.length) {
      preparedTools = prepareToolsWithUsageControl(tools, request.tools, logger, 'nvidia')
    ...
    try {
      ...
      const providerEndTimeISO = new Date(providerEndTime).toISOString()
      const totalDuration = providerEndTime - providerStartTime
      logger.error('Error in NVIDIA request:', {
        error: error instanceof Error ? error.message : String(error),
        duration: totalDuration,
      })
